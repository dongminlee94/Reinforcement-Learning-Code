...
def train_model(net, target_net, optimizer, mini_batch, batch_size):
    ...
    q_value = net(torch.Tensor(states)).squeeze(1)

    one_hot_action = torch.zeros(batch_size, q_value.size(-1))
    one_hot_action.scatter_(1, actions.unsqueeze(1), 1)
    q_value = torch.sum(q_value.mul(one_hot_action), dim=1)

    next_q_value = target_net(torch.Tensor(next_states)).squeeze(1)

    target_q_value = rewards + masks * args.gamma * next_q_value.max(1)[0]
...
def get_action(epsilon, q_value, action_size):
...
def main():
    ...
    for episode in range(args.max_iter_num):
        ...
        while not done:
            ...
            action = get_action(args.epsilon, q_value, action_size)
